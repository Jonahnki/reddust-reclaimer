...
class MarsConstraintEngine:
    ...
    def apply_mars_constraints(
        self,
        model: cobra.Model,
        conditions: MarsEnvironmentalConditions = None,
        severity: str = "realistic",
    ) -> cobra.Model:
        """
        ...
        logger.info(
            f"Location: {conditions.location.value}, Season: {conditions.season.value}"
        )
    ...
    def _apply_temperature_constraints(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions, severity: str
    ) -> None:
        """Apply temperature-based constraints to enzymatic reactions."""

        logger.info(
            f"Applying temperature constraints ({conditions.temperature_celsius}°C)"
        )
        ...
        temp_factor = np.exp(
            -activation_energy
            / R
            * (1 / (mars_temp + 273.15) - 1 / (earth_temp + 273.15))
        )
        ...
        logger.info(
            f"Applied temperature factor: {temp_factor:.3f}, efficiency: {efficiency:.3f}"
        )

    def _apply_atmospheric_constraints(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions, severity: str
    ) -> None:
        ...
        if "EX_o2_e" in model.reactions:
            o2_availability = (
                conditions.o2_partial_pressure / 0.21
            )  # Fraction of Earth O2
            model.reactions.EX_o2_e.lower_bound *= o2_availability
            logger.info(
                f"Reduced O2 availability to {o2_availability:.4f} of Earth levels"
            )
    ...
    def _apply_water_constraints(
        ...
        if "EX_h2o_e" in model.reactions:
            ...
            model.reactions.EX_h2o_e.lower_bound = max(
                -10, original_bound * water_factor
            )
            logger.info(
                f"Reduced water availability: {original_bound} -> {model.reactions.EX_h2o_e.lower_bound}"
            )
    ...
    def _apply_radiation_constraints(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions, severity: str
    ) -> None:
        """Apply radiation damage constraints."""

        logger.info(
            f"Applying radiation constraints ({conditions.cosmic_radiation} mSv/day)"
        )

        # Radiation damage factor
        radiation_damage = min(
            1.0,
            conditions.cosmic_radiation / self.stress_thresholds["radiation_damage"],
        )
    ...
    def _apply_chemical_constraints(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions, severity: str
    ) -> None:
        ...
        if "EX_clo4_e" in model.reactions:
            perchlorate_availability = (
                -conditions.perchlorate_concentration * 20
            )  # mmol/gDW/h
    ...
    def _add_stress_response_reactions(
        self, model: cobra.Model, stress_factors: Dict
    ) -> None:
                    ...
                    cold_response.add_metabolites(
                        {
                            ...
                            model.metabolites.pi_c: (
                                5 if "pi_c" in model.metabolites else 0
                            ),
                        }
                    )
                    ...
                    osmotic_response.add_metabolites(
                        {
                            ...
                            model.metabolites.adp_c: (
                                2 if "adp_c" in model.metabolites else 0
                            ),
                        }
                    )
    ...
    def _add_mars_exchanges(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions
    ) -> None:
        """Add Mars-specific exchange reactions."""
    ...
    def _validate_mars_constraints(
        self, model: cobra.Model, conditions: MarsEnvironmentalConditions
    ) -> None:
        ...
        try:
            ...
            if solution.status == "optimal":
                ...
                if growth_rate > 0:
                    logger.info(
                        f"Mars constraints validation successful - growth rate: {growth_rate:.6f}"
                    )
                    ...
                    if "PCR" in model.reactions:
                        ...
                        if pcr_flux > 0:
                            logger.info(
                                f"Perchlorate detoxification active - flux: {pcr_flux:.6f}"
                            )
                        else:
                            logger.warning(
                                "Perchlorate detoxification inactive under Mars conditions"
                            )
                else:
                    logger.warning("No growth possible under applied Mars conditions")
            else:
                logger.warning(
                    f"Mars constraints validation failed - status: {solution.status}"
                )

        except Exception as e:
            logger.error(f"Mars constraints validation error: {e}")

    def generate_seasonal_conditions(
        self, base_location: MarsLocation = MarsLocation.EQUATORIAL
    ) -> Dict[MarsSeason, MarsEnvironmentalConditions]:
        ...
        for season, modifiers in seasonal_modifiers.items():
            conditions = MarsEnvironmentalConditions(
                ...
                atmospheric_pressure=self.base_conditions.atmospheric_pressure
                * modifiers["pressure_factor"],
                season=season,
    ...
    def analyze_survival_envelope(self, model: cobra.Model) -> Dict:
        ...
        locations = [
            MarsLocation.EQUATORIAL,
            MarsLocation.POLAR,
            MarsLocation.UNDERGROUND,
        ]
            ...
            for severity in severities:
                for location in locations:
                    ...
                    test_conditions = MarsEnvironmentalConditions(
                        temperature_celsius=temp, location=location
                    )

                    try:
                        # Apply constraints
                        test_model = self.apply_mars_constraints(
                            model, test_conditions, severity
                        )
...
def main():
    ...
    for season, conditions in seasonal_conditions.items():
        logger.info(
            f"{season.value}: T={conditions.temperature_celsius}°C, P={conditions.atmospheric_pressure:.4f} atm"
        )
