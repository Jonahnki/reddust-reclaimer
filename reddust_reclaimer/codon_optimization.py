...
class MarsCodonOptimizer:
    ...
    def optimize_for_mars_conditions(self, dna_sequence: str) -> str:
        ...
        logger.info(
            f"Optimization complete: CAI improved from {original_cai:.3f} to {optimized_cai:.3f}"
        )
...
Examples:
  ...
  python codon_optimization.py --analyze --sequence ATGAAATTTGGGTAG
    ...
    parser.add_argument(
        "--sequence", help="DNA sequence to optimize (must be divisible by 3)"
    )
    parser.add_argument("--file", help="Input FASTA file with sequences to optimize")
    parser.add_argument("--output", help="Output file for optimized sequences")
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze sequence without optimization"
    )
    ...
    try:
        ...
        if args.sequence:
            ...
            if not args.analyze:
                ...
                print(
                    f"  Mars Adaptation Index: {optimized_analysis['mars_adaptation_index']}"
                )
