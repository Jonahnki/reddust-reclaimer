...
class MarsEnzymeDocking:
    ...
    def __init__(self, temperature: float = 233.15, pressure: float = 0.006):
        ...
        logger.info(
            f"Initialized Mars docking conditions: T={temperature}K, P={pressure}atm"
        )
    ...
    def dock_mars_enzyme_substrate(
        self, enzyme_name: str = "carbonic_anhydrase"
    ) -> dict:
        """
        ...
        # Calculate temp_factor once for use in mars_adapted_score
        temp_factor = 298.15 / self.temperature
        ...
        for i, ligand in enumerate(ligands):
            try:
                ...
                logger.debug(
                    f"Docked ligand {i}: {smiles} (ΔG = {affinity:.3f} kcal/mol)"
                )
        ...
        logger.info(
            f"Docking complete: {len(results['ligand_results'])} successful results"
        )
        return results
    ...
    try:
        ...
        with open(output_file, "w") as f:
            ...
            for ligand in results["ligand_results"]:
                f.write(
                    f"{ligand['smiles']}\t{ligand['binding_affinity']:.3f}\t{ligand['molecular_weight']:.1f}\n"
                )
