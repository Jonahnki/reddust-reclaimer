...
class MarsCodonOptimizer:
        ...
        for motif in problematic_motifs:
            if motif in modified_seq:
                ...
                modified_seq = modified_seq.replace(
                    motif, self._generate_alternative_sequence(motif)
                )
    ...
    def _optimize_gc_content(self, sequence: str) -> str:
        ...
        for i in range(0, len(sequence) - window_size + 1, window_size):
            ...
            if not (self.gc_target_range[0] <= current_gc <= self.gc_target_range[1]):
                logger.info(
                    f"Adjusting GC content in window {i//window_size + 1}: {current_gc:.1f}%"
                )
    ...
    def _calculate_metrics(self, original: Seq, optimized: Seq, protein: Seq) -> Dict:
        ...
        if self.cai_index:
            ...
            metrics["cai_improvement"] = (
                metrics["optimized_cai"] - metrics["original_cai"]
            )

        # Sequence similarity
        metrics["sequence_identity"] = self._calculate_identity(
            str(original), str(optimized)
        )

        # Codon changes
        original_codons = [str(original[i : i + 3]) for i in range(0, len(original), 3)]
        optimized_codons = [
            str(optimized[i : i + 3]) for i in range(0, len(optimized), 3)
        ]

        changed_codons = sum(
            1 for o, n in zip(original_codons, optimized_codons) if o != n
        )
    ...
    def _generate_summary(self, metrics: Dict) -> str:
        ...
        summary_lines.append(
            f"Sequence length: {metrics['length']} bp ({metrics['protein_length']} aa)"
        )
        summary_lines.append(
            f"GC content: {metrics['original_gc']:.1f}% → {metrics['optimized_gc']:.1f}%"
        )
        ...
        summary_lines.append(
            f"Codons changed: {metrics['codons_changed']} ({metrics['percent_changed']:.1f}%)"
        )
        summary_lines.append(f"Sequence identity: {metrics['sequence_identity']:.1f}%")
        summary_lines.append(
            f"Mars suitability score: {metrics['mars_suitability_score']:.1f}/100"
        )
...
def optimize_gene_file(
    input_file: Path, output_dir: Path, gene_name: str = None
) -> Dict:
    """
