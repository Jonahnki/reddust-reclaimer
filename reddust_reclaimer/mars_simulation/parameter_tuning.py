...
class ParameterTuner:
    ...
    def _calculate_performance_metrics(
        ...
        performance["survival_probability"] = (
            ...
            * (
                ...
                + conditions.get("osmotic_stress_proteins", 0.5) * (1 - water_stress)
            )
            / 2
        )
    ...
    def plot_optimization_results(
        self, pareto_solutions: List[Dict[str, float]], save_path: Optional[str] = None
    ) -> None:
        """
        ...
        survival_probs = [
            sol["performance"]["survival_probability"] for sol in pareto_solutions
        ]
        remediation_effs = [
            sol["performance"]["remediation_efficiency"] for sol in pareto_solutions
        ]
        metabolic_acts = [
            sol["performance"]["metabolic_activity"] for sol in pareto_solutions
        ]
        ...
        axes[1, 0].boxplot(
            [param_values[name] for name in param_names], labels=param_names
        )
    ...
    def save_optimization_results(
        self, pareto_solutions: List[Dict[str, float]], filepath: str
    ) -> None:
        """
...
def main():
    ...
    tuner.save_optimization_results(
        pareto_solutions, "data/parameter_optimization_results.json"
    )
    ...
    if pareto_solutions:
        ...
        best_growth = max(
            pareto_solutions, key=lambda x: x["performance"]["growth_rate"]
        )
        best_survival = max(
            pareto_solutions, key=lambda x: x["performance"]["survival_probability"]
        )
        best_remediation = max(
            pareto_solutions, key=lambda x: x["performance"]["remediation_efficiency"]
        )

        print(f"\nBest growth rate: {best_growth['performance']['growth_rate']:.3f}")
        print(
            f"Best survival probability: {best_survival['performance']['survival_probability']:.3f}"
        )
        print(
            f"Best remediation efficiency: {best_remediation['performance']['remediation_efficiency']:.3f}"
        )
