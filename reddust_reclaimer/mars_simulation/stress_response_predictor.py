...
class StressResponsePredictor:
    ...
    def _calculate_stress_responses(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        perchlorate_response = self.params.perchlorate_reduction_efficiency * np.exp(
            -perchlorate_stress
        )
    ...
    def analyze_stress_interactions(
        ...
        cold_osmotic_corr = correlation_matrix.loc[
        ...
        oxidative_perchlorate_corr = correlation_matrix.loc[
    ...
    def plot_stress_responses(
        ...
        axes[0, 0].scatter(
            features["temperature_c"], predictions["cold_shock_survival"], alpha=0.6
        )
        ...
        axes[0, 1].scatter(
            features["water_activity"], predictions["osmotic_stress_growth"], alpha=0.6
        )
        ...
        axes[0, 2].scatter(
            features["uv_intensity"],
            predictions["oxidative_stress_response"],
            alpha=0.6,
        )
        ...
        axes[1, 0].scatter(
            features["perchlorate_concentration"],
            predictions["perchlorate_reduction"],
            alpha=0.6,
        )
        ...
        axes[1, 2].scatter(
            predictions["metabolic_activity"],
            predictions["overall_survival"],
            alpha=0.6,
        )
...
def main():
    ...
    print(
        f"Average cold shock survival: {predictions['cold_shock_survival'].mean():.3f}"
    )
    print(
        f"Average osmotic stress growth: {predictions['osmotic_stress_growth'].mean():.3f}"
    )
    print(
        f"Average oxidative stress response: {predictions['oxidative_stress_response'].mean():.3f}"
    )
    print(
        f"Average perchlorate reduction: {predictions['perchlorate_reduction'].mean():.3f}"
    )
