...
class MarsEnvironmentSimulator:
    ...
    def generate_diurnal_cycle(self, sols: int = 30, resolution: str = '1H') -> pd.DataFrame:
        """
        ...
        for i, time in enumerate(time_index):
            ...
            ) * np.sin(phase - np.pi/2)  # Peak at noon
            ...
            temperature = np.clip(temperature, self.params.temp_min, self.params.temp_max)
            ...
            if 6 <= hour_of_sol <= 18:  # Daytime
                uv_base = self.params.uv_intensity * np.sin(np.pi * (hour_of_sol - 6) / 12)
    ...
    def calculate_stress_factors(self, env_data: pd.DataFrame) -> pd.DataFrame:
        ...
        pressure_stress = 1 - (stress_data['pressure_kpa'] / 101.325)  # Normalized to Earth pressure
        ...
        perchlorate_stress = stress_data['perchlorate_concentration'] / 0.01  # Normalized to 1% threshold
        stress_data['perchlorate_stress'] = np.clip(perchlorate_stress, 0, 1)
        
        # Combined stress index
        stress_factors = [
            'temperature_stress', 'pressure_stress', 'uv_stress', 
            'water_stress', 'perchlorate_stress'
        ]
    ...
    def simulate_seasonal_variations(self, earth_years: int = 2) -> pd.DataFrame:
        ...
        seasonal_variation = 10 * np.sin(2 * np.pi * days / (687 * 24.6))  # 687 sols per Mars year
    ...
    def plot_environmental_conditions(self, env_data: pd.DataFrame, 
    ...
    def export_environment_data(self, env_data: pd.DataFrame, 
                              filepath: str) -> None:
        """
...
def main():
    ...
    simulator.export_environment_data(seasonal_stress, 'data/mars_environment_seasonal.csv')
    
    # Print summary statistics
    print("\n=== Mars Environment Summary ===")
    print(f"Temperature range: {env_data['temperature_c'].min():.1f}°C to {env_data['temperature_c'].max():.1f}°C")
    ...
    print(f"Perchlorate concentration: {env_data['perchlorate_concentration'].iloc[0]:.3f}%")
