...
class GeneSearchEngine:
    ...
    def search_ncbi_nucleotide(self, gene_name: str, organism: str = None, max_results: int = 50) -> List[Dict]:
        """
                        ...
                        if feature.get('GBFeature_key') == 'CDS':
                            ...
                            for qual in qualifiers:
                                if qual.get('GBQualifier_name') == 'product':
                                    record_info['product'] = qual.get('GBQualifier_value', '')
                                elif qual.get('GBQualifier_name') == 'gene':
                                    record_info['gene_symbol'] = qual.get('GBQualifier_value', '')
    ...
    def search_uniprot(self, gene_name: str, organism: str = None, max_results: int = 50) -> List[Dict]:
        """
                ...
                try:
                    ...
                    for comment in comments:
                        if comment.get('commentType') == 'FUNCTION':
                            record_info['function'] = comment.get('texts', [{}])[0].get('value', '')
                        elif comment.get('commentType') == 'SUBCELLULAR LOCATION':
                            record_info['subcellular_location'] = comment.get('texts', [{}])[0].get('value', '')
                        elif comment.get('commentType') == 'TEMPERATURE DEPENDENCE':
                            record_info['temperature_dependence'] = comment.get('texts', [{}])[0].get('value', '')
    ...
    def score_gene_suitability(self, record: Dict) -> float:
        ...
        cold_keywords = ['psychro', 'cold', 'antarctic', 'arctic', 'low temperature', 'freeze', 'frost']
        salt_keywords = ['halo', 'salt', 'saline', 'osmotic', 'high salt']
        stress_keywords = ['stress', 'extreme', 'harsh', 'resistant', 'tolerance']
        
        description_text = (record.get('definition', '') + ' ' + 
                          record.get('function', '') + ' ' +
                          record.get('temperature_dependence', '')).lower()
    ...
    def comprehensive_gene_search(self) -> Dict[str, List[Dict]]:
        ...
        """
        logger.info("Starting comprehensive gene search for perchlorate detoxification pathway")
        ...
        for gene_code, gene_description in self.target_genes.items():
            ...
            for organism in self.target_organisms:
                ...
                ncbi_results = self.search_ncbi_nucleotide(gene_description, organism, max_results=10)
            ...
            logger.info(f"Found {len(unique_results)} unique candidates for {gene_code}")
    ...
    def _create_overall_summary(self, search_results: Dict[str, List[Dict]]) -> None:
        ...
        for gene_code, candidates in search_results.items():
            gene_summary = {
                ...
                'candidates_with_sequences': len([c for c in candidates if c.get('sequence')]),
                'top_organism': candidates[0].get('organism', '') if candidates else '',
                'top_score': candidates[0].get('suitability_score', 0) if candidates else 0,
                'source_distribution': {}
            }
            ...
            for candidate in candidates:
                ...
                gene_summary['source_distribution'][source] = gene_summary['source_distribution'].get(source, 0) + 1
...
def main():
    ...
    for gene_code, candidates in results.items():
        ...
        if candidates:
            ...
            logger.info(f"  Top candidate: {top_candidate.get('organism', 'Unknown')} "
