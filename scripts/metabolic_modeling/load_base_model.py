...
class BSubtilisModelLoader:
    ...
    def load_model(self, model_name: str = 'iYO844', force_download: bool = False) -> cobra.Model:
        """
        ...
        # Download if needed
        if force_download or not (self.model_dir / self.available_models[model_name]['file']).exists():
    ...
    def _validate_model(self, model: cobra.Model) -> None:
        ...
        try:
            ...
            if solution.status != 'optimal':
                ...
                logger.info(f"Model validation successful, growth rate: {solution.objective_value:.3f}")
    ...
    def analyze_model(self, model: cobra.Model = None) -> Dict:
        ...
        analysis['reaction_stats'] = {
            ...
            'transport': len([r for r in model.reactions if len(set([m.compartment for m in r.metabolites])) > 1]),
        ...
        try:
            ...
            with model:
                ...
                for rxn in model.exchanges:
                    if rxn.id not in ['EX_glc__D_e', 'EX_pi_e', 'EX_nh4_e', 'EX_so4_e', 'EX_mg2_e', 'EX_fe3_e']:
                        rxn.lower_bound = 0
    ...
    def _analyze_pathway_coverage(self, model: cobra.Model) -> Dict:
        ...
        key_pathways = {
            ...
            'tca_cycle': ['CS', 'ACONT', 'ICDH', 'AKGDH', 'SUCOAS', 'SUCD', 'FUM', 'MDH'],
    ...
    def apply_mars_constraints(self, model: cobra.Model = None) -> cobra.Model:
        ...
        if 'EX_o2_e' in model.reactions:
            ...
            model.reactions.EX_o2_e.lower_bound = max(-1, original_o2 * 0.001)  # 0.1% of Earth oxygen
            logger.info(f"Reduced O2 availability: {original_o2} -> {model.reactions.EX_o2_e.lower_bound}")
        ...
        if 'EX_h2o_e' in model.reactions:
            ...
            model.reactions.EX_h2o_e.lower_bound = max(-10, original_h2o * 0.1)  # 10% of normal water
            logger.info(f"Reduced H2O availability: {original_h2o} -> {model.reactions.EX_h2o_e.lower_bound}")
    ...
    def save_model_analysis(self, analysis: Dict, filename: str = None) -> Path:
        ...
        if filename is None:
            filename = f"model_analysis_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def visualize_model_stats(self, analysis: Dict) -> None:
        ...
        ax2.pie(reaction_stats.values(), labels=reaction_stats.keys(), autopct='%1.1f%%')
        ...
        # Growth conditions
        ax4 = axes[1, 1]
        if 'growth_analysis' in analysis and isinstance(analysis['growth_analysis'], dict):
            growth_data = {k: v for k, v in analysis['growth_analysis'].items() 
...
def main():
    ...
    mars_analysis_file = loader.save_model_analysis(mars_analysis, 'mars_model_analysis.json')
