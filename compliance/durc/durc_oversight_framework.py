...
class DURCRiskAssessment:
    ...
    def assess_application_risk(self) -> Dict[str, float]:
        ...
        application_risk = (
            dual_use_risk + weaponization_risk + proliferation_risk
        ) / 3.0
...
class ExportControlCompliance:
    ...
    def generate_export_control_report(self) -> Dict[str, any]:
        ...
        overall_compliance = (
            not itar_compliance["compliance_required"]
            and ear_compliance["ear_controlled"]
            and not wassenaar_compliance["wassenaar_controlled"]
        )
...
class ResponsiblePublicationProtocol:
    ...
    def generate_technology_transfer_protocol(
        self, recipient_institution: str
    ) -> Dict[str, any]:
        """
        ...
        is_authorized = any(
            inst.lower() in recipient_institution.lower()
...
class DURCOversightFramework:
    ...
    def conduct_comprehensive_durc_assessment(
        ...
        publication_risk = self.publication_protocols.assess_publication_risk(
            research_description
        )

        # Technology transfer protocol
        tech_transfer_protocol = (
            self.publication_protocols.generate_technology_transfer_protocol(
        ...
        overall_approval = (
            ...
            and ibc_review["approval_status"] in ["APPROVED", "CONDITIONAL_APPROVAL"]
            and export_control_report["overall_compliance"]
            and publication_risk["publication_status"] != "RESTRICTED"
        )
    ...
    def plot_durc_assessment_metrics(
        self, assessment_data: Dict[str, any], save_path: Optional[str] = None
    ) -> None:
        """
        ...
        colors = [
            "red" if v > 0.5 else "orange" if v > 0.3 else "green" for v in risk_values
        ]
        ...
        pub_values = [
            pub_data["risk_score"],
            1 if pub_data["publication_status"] == "APPROVED" else 0.5,
        ]

        colors = [
            "green" if v > 0.8 else "orange" if v > 0.5 else "red" for v in pub_values
        ]
    ...
    def save_durc_assessment_report(
        self, assessment_data: Dict[str, any], filepath: str
    ) -> None:
        """
...
def main():
    ...
    assessment_data = durc_framework.conduct_comprehensive_durc_assessment(
        research_description
    )
    ...
    durc_framework.save_durc_assessment_report(
        assessment_data, "compliance/durc/durc_assessment_report.json"
    )
    ...
    print(
        f"Export Control Compliance: {assessment_data['export_control_compliance']['overall_compliance']}"
    )
    print(
        f"Publication Status: {assessment_data['publication_risk']['publication_status']}"
    )
